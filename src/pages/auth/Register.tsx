...
export default function Register() {
  ...
  const onSubmit = async (data: TRegisterForm) => {
    ...
    try {
      ...
      console.log({res})
      toast.success(res.message || "Registered successfully", { id: toastId, duration: 2000 });
      navigate("/login");
    } catch (err: any) {
      console.log({ err });
      ...
      <div className="flex flex-col gap-5 md:col-span-2 lg:col-span-1 bg-liquidGreen p-10">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
          <div className="form-group flex flex-col gap-3 lg:px-10">
            ...
            <Input id="firstName" placeholder="John" {...register("first_name", { required: "First name is required" })} />
          ...
          <div className="form-group flex flex-col gap-3 lg:px-10">
            ...
            <Input id="lastName" placeholder="Doe" {...register("surname", { required: "Last name is required" })} />
            {errors.surname && <span className="text-red-500 text-sm">{errors.surname.message}</span>}
          </div>

          <div className="form-group flex flex-col gap-3 lg:px-10">
            <Label htmlFor="email" className="text-secondary">Email</Label>
            <Input id="email" type="email" placeholder="jon.doe@example.com" {...register("email", { required: "Email is required" })} />
            {errors.email && <span className="text-red-500 text-sm">{errors.email.message}</span>}
          </div>

          <div className="form-group flex flex-col gap-3 lg:px-10">
            <Label htmlFor="telephone" className="text-secondary">Telephone</Label>
            <PhoneInput
              country={"ma"}
              value={phone}
              onChange={setPhone}
          ...
          <div className="form-group flex flex-col gap-3 lg:px-10">
            ...
            <div className="relative w-full">
              ...
              />
              <Separator className="bg-primary/50 w-0.5 h-[70%] absolute top-1/2 right-12 -translate-y-1/2" orientation="vertical" />
              {showPassword ? (
                <EyeOff onClick={() => setShowPassword(false)} className="absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer text-primary/50" />
              ) : (
                <Eye onClick={() => setShowPassword(true)} className="absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer text-primary/50" />
              )}
            </div>
          ...
          <div className="form-group flex flex-col gap-3 lg:px-10">
            ...
            <div className="relative w-full">
              ...
              <Separator className="bg-primary/50 w-0.5 h-[70%] absolute top-1/2 right-12 -translate-y-1/2" orientation="vertical" />
              {showConfirmPassword ? (
                <EyeOff onClick={() => setShowConfirmPassword(false)} className="absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer text-primary/50" />
              ) : (
                <Eye onClick={() => setShowConfirmPassword(true)} className="absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer text-primary/50" />
              )}
            </div>
        ...
        <div className="form-group w-full flex items-center justify-center lg:px-10">
          <Button className="w-full flex gap-3 items-center rounded-full" size="lg" variant="transparent">
